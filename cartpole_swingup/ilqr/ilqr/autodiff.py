...
def jacobian_scalar(expr, wrt):
    ...
    Args:
        expr: Scalar aesara tensor expression.
        wrt: List of aesara variables.
...
def jacobian_vector(expr, wrt, size):
    ...
    Args:
        expr: Vector aesara tensor expression.
        wrt: List of aesara variables.
        size: Vector size.
...
def as_function(expr, inputs, **kwargs):
    """Converts and optimizes a aesara expression into a function.

    Args:
        expr: aesara tensor expression.
        inputs: List of aesara variables to use as inputs.
        **kwargs: Additional key-word arguments to pass to `aesara.function()`.

    Returns:
        A function.
    """
    return aesara.function(inputs, expr, on_unused_input="ignore", **kwargs)
